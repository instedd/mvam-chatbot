...
describe ::MvamBot::Bot do
  describe "rescheduling surveys" do
    ...
    it "should run surveys for scheduled users" do
      MvamBot::Spec::DB.cleanup
      user1 = Factory::DB.user(telegram_id: 1000, survey_at: Time.utc_now - 1.day)
      user2 = Factory::DB.user(telegram_id: 1001, survey_at: Time.utc_now - 1.hour)
      user3 = Factory::DB.user(telegram_id: 1002, survey_at: Time.utc_now + 1.hour)
      user4 = Factory::DB.user(telegram_id: 1003, survey_at: nil)
      ...
      MvamBot::User.find_telegram(1000).not_nil!.survey_at.should eq(nil)
      MvamBot::User.find_telegram(1001).not_nil!.survey_at.should eq(nil)
      MvamBot::User.find_telegram(1002).not_nil!.survey_at.not_nil!.should be_close(Time.utc_now + 1.hour, 1.minute)
      MvamBot::User.find_telegram(1003).not_nil!.survey_at.should eq(nil)
    end
  end
end
